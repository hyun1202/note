...
* 메서드 영역
	...
	* **static 영역**: `static`변수 보관
		* 특정 클래스에서 공용으로 사용할 수 있는 변수 및 메서드 정보
		* JVM이 종료될 때 까지 생존
	* **런타임 상수 풀**: 프로그램을 실행하는데 필요한 공통 리터럴 상수 보관
* 스택 영역
	* **각 쓰레드 별로 하나의 실행 스택** 생성
	* 각 스택 프레임은 지역 변수, 중간 연산 결과, 메서드 호출 정보 등을 포함
		* **스택 프레임**: 메서드를 호출할 때마다 하나의 스택 프레임이 쌓이고, 메서드가 종료되면 해당 스택 프레임이 제거
		* **지역 변수 생명 주기**: 메서드가 종료되면 지역 변수도 함께 제거된다.
* 힙 영역
	* 객체와 배열이 생성되는 영역
